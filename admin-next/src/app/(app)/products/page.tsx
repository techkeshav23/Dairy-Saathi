...
type P = { id: string; name: string; displayName: string; cat: string; mrp: number; rate: number; resale: number; eaPerKg: number; eaPerCrate: number; cratePrice: number; moq: number; stock: number; pack: string; image: string; slabs?: { min_qty: number; price_per_unit: number }[] };

const TINTS = [["#eef2fe", "#2b50d6"], ["#faf0dc", "#c07708"], ["#e6f6ee", "#0f9d63"], ["#f1f3f7", "#586172"]];
const BLANK: P = { id: "", name: "", displayName: "", cat: "", mrp: 0, rate: 0, resale: 0, eaPerKg: 0, eaPerCrate: 0, cratePrice: 0, moq: 1, stock: 0, pack: "1 EA", image: "", slabs: [{ min_qty: 1, price_per_unit: 0 }] };
...
export default function ProductsPage() {
  ...
  const NUM: (keyof P)[] = ["mrp", "rate", "resale", "eaPerKg", "eaPerCrate", "cratePrice", "moq", "stock"];
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={() => setModal(null)}>
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 max-h-[70vh] overflow-y-auto">
              ...
              {(["mrp", "resale", "eaPerKg", "eaPerCrate", "cratePrice", "moq", "stock", "pack"] as (keyof P)[]).map((key) => (
                <label key={key} className="block">
                  <span className="mb-1 block text-[12px] font-medium capitalize text-muted">{key === "pack" ? "Pack" : key === "eaPerKg" ? "EA per KG (0 = no KG)" : key === "eaPerCrate" ? "Pieces per Crate (0 = no crate)" : key === "cratePrice" ? "Crate Price ₹" : key}</span>
                  <input
